...
2. 소셜 회원가입
    ...
    2-3. 사용자 필수정보이용 동의 후 엑세스토큰발행, 이 토큰을 이용해 Naver( or Kakao )에 사용자 정보를 받아와 회원가입을 진행 합니다.
...
4-1. 이메일 인증을 하지 않고 로그인시 인증 요청 페이지로 이동 합니다.
...
1. 상춤 추가
    ...
    1-2. 로그인 정보가 없는 상태에서 추가 페이지로의 접속을 제한합니다. 이는 **JSP** 단에서 버튼을 보여주지 않으며, **JAVA**의 **인터셉터**를 통해 다시 한번 제한합니다 
